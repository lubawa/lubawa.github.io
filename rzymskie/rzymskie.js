...
let cyfry_rzymskie=["I","V","X","L","C","D","M"]
let tresc_diva ="";
	
	for (i=0; i<=6; i++)
	{
		tresc_diva = tresc_diva + `<div Id="klawisz`+i+`" class="klawisz" onclick="pisz('`+cyfry_rzymskie[i]+`')">`+cyfry_rzymskie[i]+`</div>`;
	}
    ...
    function losuj(a,b) 
{
    ...
    document.getElementById("losowanie").innerHTML = los;
...
function sprawdz()
{
    ...
    if(arab_to_rzym(los)==document.getElementById("ekran").innerText)
    {
    ...
    else
    {
        ...
        bledy.push([document.getElementById("losowanie").innerText,document.getElementById("ekran").innerText, arab_to_rzym(los)]);
    }
    if(pytanie==5) 
    {
    zakoncz();
